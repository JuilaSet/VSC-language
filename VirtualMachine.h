...
class VirtualMachine
{
	...
	// 保存所在语句块数据栈大小恢复值
	std::vector<unsigned int> blk_stk;
...
public:
	VirtualMachine() :_stop_val(0) , _flag_has_instruct(false) { };
	VirtualMachine(vec_command_t instruct) :instruct(instruct) ,_stop_val(0) , _flag_has_instruct(true) { }
	~VirtualMachine();

	// 设置执行代码
	void setInstruct(vec_command_t ins);
...
#endif
		vm->push_local_list();
		vm->blk_stk.push_back(vm->stk.size());
#if CHECK_Eval 
