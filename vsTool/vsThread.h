...
namespace vsThread {
	...
	public:
		...
		void eval(vsTool::id_t first, KEY_TYPE key) {
	...
	public:
		...
		// 构建结点
		void build_node(node_mapping_func<KEY_TYPE, DATA_TYPE> evalFunc, const std::initializer_list<KEY_TYPE>& keys, vsTool::id_t id) {
			auto task_ptr = _thread_node_ptr<vsTool::id_t, KEY_TYPE, DATA_TYPE>
				(new _Thread_Node<vsTool::id_t, KEY_TYPE, DATA_TYPE>(evalFunc, keys, id));
